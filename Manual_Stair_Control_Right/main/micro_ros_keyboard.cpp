...
rcl_subscription_t keyboard_sub_group1;
std_msgs__msg__Int8 keyboard_msg_group1;

// Mutex for protecting micro-ROS network operations (WiFi/UDP)
SemaphoreHandle_t microros_network_mutex;

void keyboard_callback_group1(const void *msgin)
{
    const std_msgs__msg__Int8 *msg = (const std_msgs__msg__Int8 *)msgin;
    int8_t command = msg->data;
    
    ESP_LOGI(TAG, "Received Group 1 keyboard command: %d", command);
    ...
    if (xSemaphoreTake(motor_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        ...
        switch (command) {
            case 1:
                ESP_LOGI(TAG, "Motor 1 Command: Rotate clockwise");
                break;
            case -1:
                ESP_LOGI(TAG, "Motor 1 Command: Rotate counter-clockwise");
                break;
            case 0:
                ESP_LOGI(TAG, "Motor 1 Command: Stop motor");
                break;
            default:
                ESP_LOGW(TAG, "Motor 1 Unknown command: %d, stopping motor", command);
                motor2_command = 0;
                break;
        }
    } else {
        ESP_LOGW(TAG, "Failed to acquire motor mutex for Motor 1");
    }
}
...
void micro_ros_init(void)
{
    ...
    // Initialize messages
    memset(&keyboard_msg_group1, 0, sizeof(keyboard_msg_group1));
    ...
    RCCHECK(rclc_node_init_default(&node, "keyboard_motor_group1_controller", "", &support));
    
    // Initialize subscriber for Group 1
    ...
    RCCHECK(rclc_publisher_init_default(
        ...
        "/as5600_sensors_right"
    ));
    
    ESP_LOGI(TAG, "AS5600 sensor publisher initialized on topic: /as5600_sensors_right");
    ...
    RCCHECK(rclc_publisher_init_default(
        ...
        &node,
        ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32MultiArray),
        "/encoder_sensors_right"
    ));
    
    ESP_LOGI(TAG, "Encoder publisher initialized on topic: /encoder_sensors_right");
    
    // Initialize executor with 1 subscription
    executor = rclc_executor_get_zero_initialized_executor();
    RCCHECK(rclc_executor_init(&executor, &support.context, 1, &allocator));
    
    // Add Group 1 subscription
    RCCHECK(rclc_executor_add_subscription(
        &executor, 
        &keyboard_sub_group1, 
        &keyboard_msg_group1, 
        &keyboard_callback_group1, 
        ON_NEW_DATA
    ));
    
    ESP_LOGI(TAG, "micro-ROS initialized successfully");
    ESP_LOGI(TAG, "Subscribed to topic: /keyboard/group1");
    ESP_LOGI(TAG, "Publishing AS5600 sensor data to: /as5600_sensors_right");
}
...
void micro_ros_spin_task(void *arg)
{
    ...
    while (1) {
        ...
        float radians_motor1 = 0.0f;
        float rpm_motor1 = 0.0f;
        
        if (g_as5600_0) {
            radians_motor1 = g_as5600_0->get_radians();
            rpm_motor1 = g_as5600_0->get_rpm();
        }
        
        // 3. Publish sensor data
        if (xSemaphoreTake(sensor_msg_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
            // Populate message data
            sensor_msg.data.data[0] = radians_motor1;  // Motor 1 angle in radians
            sensor_msg.data.data[1] = rpm_motor1;       // Motor 1 RPM
