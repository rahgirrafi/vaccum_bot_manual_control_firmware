...
void micro_ros_init(void)
{
    ...
    RCCHECK(rclc_node_init_default(&node, "keyboard_motor_group2_group3_controller", "", &support));
    ...
    ESP_LOGI(TAG, "AS5600 sensor publisher initialized on topic: /as5600_sensors");
...
void micro_ros_spin_task(void *arg)
{
    ...
    // Data format: [motor2_angle_rad, motor2_rpm, motor3_angle_rad, motor3_rpm]
    ...
    while (1) {
        ...
        float radians_motor3 = 0.0f;
        float rpm_motor3 = 0.0f;
        
        if (g_as5600_1) {
            radians_motor3 = g_as5600_1->get_radians();
            rpm_motor3 = g_as5600_1->get_rpm();
        }
        
        // 3. Publish sensor data
        if (xSemaphoreTake(sensor_msg_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
            // Populate message data
            sensor_msg.data.data[0] = radians_motor2;  // Motor 2 angle in radians
            sensor_msg.data.data[1] = rpm_motor2;       // Motor 2 RPM
            sensor_msg.data.data[2] = radians_motor3;  // Motor 3 angle in radians
            sensor_msg.data.data[3] = rpm_motor3;       // Motor 3 RPM
        ...
        if (log_counter % 50 == 0) {
            ESP_LOGI(TAG, "AS5600 Sensors - Motor2: %.2f rad, %.1f RPM | Motor3: %.2f rad, %.1f RPM", 
                     radians_motor2, rpm_motor2, radians_motor3, rpm_motor3);
